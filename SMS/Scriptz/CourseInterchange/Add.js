...
$(function () {
    ...
    form.find("#CourseInterchangeStepz").steps({
        headerTag: "h3",
        bodyTag: "section",
    ...
    GetCourseDetails = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#txtSingleCourseIds").val(data[0].CourseIds);
                ...
                var courseInterchange_ST_Fee = parseInt($("#txtCourseInterchangeST").val());
    ...
    var ValidateCourse = function () {
        ...
        if (joinedCourseCombo.length === newCourseCombo.length) {
            ...
            //Checking whether newcombination and oldcombination is same
            var is_same =  newCourseCombo.every(function (element, index) {
                return element === joinedCourseCombo[index];
            });
            ...
            if (!is_same) {
                ...
                validate = false;
                bootbox.alert("No courses interchanged.Kindly select atleast one different course from the previously selected one.");
            }

        }
        else {
            validate = false;
            bootbox.alert("Total number of courses previously selected should match with the number of courses currently selected");
        }
    ...
    var GetCallOutMessage = function () {
        ...
        var fee = parseInt($("#txtFee").val()) + parseInt($("#txtCourseInterchangeFee").val());
        var st = parseInt($("#txtSTAmt").val()) + parseInt($("#txtCourseInterchangeST").val());
